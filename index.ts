...
async function demo() {
  ...
  let searchCmdOut1Connector: ParentConnectorIface =
    searchCmd.GetOutConnector('OUTPUT1');
  let viewCmdIn1Connector: ChildConnectorIface =
    viewCmd.GetInConnector('INPUT1');
  ...
  console.log(
    "VIEW CMD 'from' after connected to SEARCH CMD :",
    viewCmd.GenerateProps()['from']
  );
  await sleep(1000);

  searchCmd.SetIntoLabel('new_search_1');
  console.log(
    "VIEW CMD 'from' after SEARCH CMD's label changed :",
    viewCmd.GenerateProps()['from']
  );
  await sleep(1000);
  ...
  if (searchCmdOut1Connector.IsConnected()) {
    console.log(
      "get VIEW CMD's coordinate from SEARCH CMD :",
      searchCmd.GetOutConnector('OUTPUT1').GetChildInfo().GetCoordinate()
    );
    console.log(
      "get VIEW CMD's uuid from SEARCH CMD :",
      searchCmd.GetOutConnector('OUTPUT1').GetChildInfo().GetID()
    );
  ...
  console.log(
    "VIEW CMD 'from' after disconnected from SEARCH CMD :",
    viewCmd.GenerateProps()['from']
  );
}
